   ...
   public class Tree : ScriptableObject
    {
        ...
        public void GenerateTrunk(Vector3Int pos, Chunk chunk, int[] blocks, System.Random random, out int height)
        {
            ...
            for (int h = 0; h < height; h++)
            {
                ...
                if (chunk.GetBlock(pos.x, pos.y + 1 + h, pos.z) == 0)
                {
                    chunk.SetBlock(trunkBlock.blockID, pos.x, pos.y + 1 + h, pos.z);
                }
            }
        }

        // Virtual so that different trees can generate different types of canopy
        public virtual void GenerateCanopy(Vector3Int pos, Chunk chunk, int[] blocks, System.Random random) { }
    }
}
